...
async function ensureAttendanceSchema() {
  try {
    ...
    await db.query(
      `ALTER TABLE faculty_assignments ADD COLUMN IF NOT EXISTS start_time TIME NULL`
    );
    await db.query(
      `ALTER TABLE faculty_assignments ADD COLUMN IF NOT EXISTS end_time TIME NULL`
    );
    await db.query(
    ...
    // Backfill assignment times from exams.exam_time when missing
    await db.query(`
