...
const ExamHallAllotment = () => {
  ...
  const handleSendNotifications = async () => {
    ...
    try {
      ...
      const msg =
        err.response?.data?.error || err.response?.data?.message || "Failed to send notification.";
      setNotificationStatus({
        message: msg,
        loading: false,
        error: true,
    ...
    const filtered = seatingPlans
      ...
      .flatMap((plan) => {
        ...
        return plan.venuesUsed.map((venue) => {
          ...
          const allCells = venue.seatingArrangement.flat();
          const allRolls = allCells.filter(
            (cell) => cell && cell !== "Empty" && typeof cell === "string"
          );

          const prefixMap = {};
          allRolls.forEach((regnNo) => {
            const match = regnNo.match(/^[0-9]*[A-Z]+/);
            if (match) {
              const prefix = match[0];
              if (!prefixMap[prefix]) prefixMap[prefix] = [];
              prefixMap[prefix].push(regnNo);
            }
          });

          const batchRows = Object.keys(prefixMap).map((prefix) => {
            const rolls = prefixMap[prefix];
            return {
              yearBranch: prefix,
    ...
    <div className="p-6 max-w-6xl mx-auto font-poppins">
      ...
      {notificationStatus.message && (
        <div
          className={`p-3 mb-4 rounded-md ${
            notificationStatus.error ? "bg-red-100 border-l-4 border-red-500" : "bg-green-100 border-l-4 border-green-500"
          }`}
        >
          <p className={`font-semibold ${notificationStatus.error ? "text-red-700" : "text-green-700"}`}>
            {notificationStatus.message}
          </p>
        </div>
      )}
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex flex-col space-y-2">
          <button
            ...
            disabled={!filters.date || !["FN", "AN"].includes(filters.session) || filteredHalls.length === 0}
      ...
      <div ref={printRef}>
        <div className="text-center mb-4">
          ...
          <p>AY 2025-2026 - ODD SEM (CAT I / CAT II)</p>
        </div>

        {Object.keys(hallsByTime).length === 0 ? (
          <p className="text-center text-gray-600">No records found for the selected filters.</p>
        ) : (
                  ...
                  {halls.map((hall, hIndex) =>
                    hall.rows.length === 0 ? (
                      <tr key={hIndex}>
                        ...
                        <td colSpan="5" className="border border-black px-3 py-2 text-center text-gray-500">
